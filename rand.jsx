...
const Landing = () => {
    ...
    //VARIABLES FOR SCROLL LOCKING
    const [justActivated, setJustActivated] = useState(false);
    const prevSection2 = useRef(false);
    const prevSection3 = useRef(false);
    const prevSection4 = useRef(false);
    const justActivatedRef = useRef(justActivated);
    const [isIOS, setIsIOS] = useState(false);
    useEffect(() => {
        justActivatedRef.current = justActivated;
    }, [justActivated]);

    //VARTIABLES FOR SCROLL LIMITER
    const scrollLimiterActive = useRef(false);
    const scrollLimitPosition = useRef(0);
    const scrollLimitThreshold = useRef(0);
    ...
    //FUNCTION TO ACTIVATE SCROLL LIMITER
    const activateScrollLimiter = (threshold) => {
        scrollLimiterActive.current = true;
        scrollLimitPosition.current = window.scrollY;
        scrollLimitThreshold.current = threshold;
    };

    //SCROLL LIMITER EFFECT
    useEffect(() => {
        if (!scrollLimiterActive.current) return;

        const handleScrollLimiter = () => {
            const currentScroll = window.scrollY;
            const scrollDelta = Math.abs(currentScroll - scrollLimitPosition.current);

            if (scrollDelta > scrollLimitThreshold.current) {
                window.scrollTo(0,
                    scrollLimitPosition.current +
                    Math.sign(currentScroll - scrollLimitPosition.current) * scrollLimitThreshold.current
                );
            }
        };

        window.addEventListener('scroll', handleScrollLimiter);
        return () => window.removeEventListener('scroll', handleScrollLimiter);
    }, []);

    //OLD: JUST ACTIVATED FLAG FOR SECTIONS
    ...
    const triggerJustActivated = () => {
        setJustActivated(true);
        activateScrollLimiter(window.innerHeight * 0.7); // 60% vh threshold
        setTimeout(() => {
            setJustActivated(false);
            scrollLimiterActive.current = false;
        }, 500);
    };
    ...
    // MODIFIED: SCROLL LOCKING MECHANISM FOR IOS COMPATIBILITY
    useEffect(() => {
        ...
        if (justActivated) {
            ...
            // Apply position fixed only on iOS
            if (isIOS) {
